...
parser.add_argument('--style', type=str, help='Path to style image or directory.', default=None)
...
parser.add_argument('--style_layers', type=int, default=4, help='Number of layers to inject style.')
parser.add_argument('--alpha', type=float, default=0.7, help='Alpha blending weight.')
...
if args.style is not None:
    if os.path.isdir(args.style):
        style_images = [
            os.path.join(args.style, file) 
            for file in os.listdir(args.style) 
            if file.endswith('.png') or file.endswith('.jpg') or file.endswith('.webp')
        ]
    else:
        style_images = [args.style]
    print(f'Total number of style images: {len(style_images)}')
else:
    style_images = [None]
    print('No style image provided.')
...
for idx, sample in enumerate(outputs):
    ...
    for id_s, style_path in enumerate(style_images):
        with torch.no_grad():
            # get triplane
            planes = model.forward_planes(images, input_cameras, style_path, args.alpha, args.style_layers)

            # determine output file name
            if style_path is not None:
                style_base = os.path.splitext(os.path.basename(style_path))[0]
                out_name = f"{name}_{style_base}"
            else:
                out_name = name

            # get mesh
            mesh_path_idx = os.path.join(mesh_path, f'{out_name}.obj')
            ...
            # get video
            if args.save_video:
                video_path_idx = os.path.join(video_path, f'{out_name}.mp4')
